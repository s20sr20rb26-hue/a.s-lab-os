...
`,
  ...
  function exportAll() {
  // localStorageの中身そのものを返す（あなたのKEYに紐づくDB）
  const raw = localStorage.getItem("lab_os_v1");
  ...
  if (!obj || !Array.isArray(obj.pages) || !Array.isArray(obj.runs)) {
    throw new Error("形式が違うっぽい（pages/runsがない）");
  }
  // 書き込み
  localStorage.setItem("lab_os_v1", JSON.stringify(obj));
  // ページ内で使ってるdbも反映（再読込が一番安全）
  location.reload();
  }
  ...
  window.Store = {
    ...
    addRun, listRuns, getRun, updateRun, deleteRun,exportAll, importAll,
  };
})();
- 培地、6well、PBS
- 染色液（必要なら [[染色液]] として作る）
